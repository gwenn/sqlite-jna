...
public class Stmt {
  ...
  public String getSql() {
    return getCString(sqlite3_sql(pStmt)); // ok if pStmt is null
  }
  ...
  public String getColumnDeclType(int iCol) throws StmtException {
    checkOpen();
    checkColumnIndex(iCol);
    return getCString(sqlite3_column_decltype(pStmt, iCol)); // ko if pStmt is null
  }
  ...
  public String getColumnName(int iCol) throws StmtException {
    ...
    columnNames[iCol] = getCString(sqlite3_column_name(pStmt, iCol)); // ko if pStmt is null
    return columnNames[iCol];
  }
  /**
   * @param iCol The leftmost column is number 0.
  ...
  public String getColumnOriginName(int iCol) throws StmtException {
    checkOpen();
    checkColumnIndex(iCol);
    return getCString(sqlite3_column_origin_name(pStmt, iCol)); // ko if pStmt is null
  }
  /**
   * @param iCol The leftmost column is number 0.
   * @return Table name
  ...
  public String getColumnTableName(int iCol) throws StmtException {
    checkOpen();
    checkColumnIndex(iCol);
    return getCString(sqlite3_column_table_name(pStmt, iCol)); // ko if pStmt is null
  }
  /**
   * @param iCol The leftmost column is number 0.
  ...
  public String getColumnDatabaseName(int iCol) throws StmtException {
    checkOpen();
    checkColumnIndex(iCol);
    return getCString(sqlite3_column_database_name(pStmt, iCol)); // ko if pStmt is null
  }

  public byte[] getColumnBlob(int iCol) throws StmtException {
    checkColumnIndex(iCol);
    final Pointer<Byte> p = sqlite3_column_blob(pStmt, iCol); // ok if pStmt is null
    if (p == null) {
      return null;
    } else {
      return p.getBytes(getColumnBytes(iCol)); // a copy is made...
    }
  }
  ...
  public String getColumnText(int iCol) throws StmtException {
    checkColumnIndex(iCol);
    return getCString(sqlite3_column_text(pStmt, iCol)); // ok if pStmt is null
  }
  ...
  public String getBindParameterName(int i) { // TODO Cache?
    return getCString(sqlite3_bind_parameter_name(pStmt, i)); // ok if pStmt is null
  }
  ...
  public void bindBlob(int i, byte[] value) throws StmtException {
    ...
    checkBind(sqlite3_bind_blob(pStmt, i, Pointer.pointerToBytes(value), value.length, SQLITE_TRANSIENT), "sqlite3_bind_blob", i);
  }
  /**
   * @param i     The leftmost SQL parameter has an index of 1
   * @param value SQL parameter value
  ...
  public void bindText(int i, String value) throws StmtException {
    ...
    Pointer<Byte> pValue = Pointer.pointerToBytes(bytes);
    // ok if pStmt is null => SQLITE_MISUSE
    checkBind(sqlite3_bind_text(pStmt, i, pValue, bytes.length, SQLITE_TRANSIENT), "sqlite3_bind_text", i);
  }
