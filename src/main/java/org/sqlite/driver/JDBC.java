...
public class JDBC implements Driver {
	...
	public Connection connect(String url, @Nullable Properties info) throws SQLException {
		...
		org.sqlite.Conn conn = org.sqlite.Conn.open(url.substring(PREFIX.length()), flags, vfs);
		SQLWarning warnings;
		try {
	...
	public DriverPropertyInfo[] getPropertyInfo(String url, @Nullable Properties info) {
		...
		DriverPropertyInfo mode = new DriverPropertyInfo(MODE, info == null ? null : info.getProperty(MODE));
		...
		DriverPropertyInfo cache = new DriverPropertyInfo(CACHE, info == null ? null : info.getProperty(CACHE));
		cache.description = "Choose to participate or not participate in shared cache mode.";
		cache.choices = new String[]{"shared", "private"};

		DriverPropertyInfo fks = new DriverPropertyInfo(FOREIGN_KEYS.name, info == null ? null : info.getProperty(FOREIGN_KEYS.name));
		fks.description = "Enable or disable the enforcement of foreign key constraints.";
		fks.choices = new String[]{"on", "off"};

		DriverPropertyInfo triggers = new DriverPropertyInfo(ENABLE_TRIGGERS.name, info == null ? null : info.getProperty(ENABLE_TRIGGERS.name));
		triggers.description = "Enable or disable triggers.";
		triggers.choices = new String[]{"on", "off"};

		DriverPropertyInfo ele = new DriverPropertyInfo(ENABLE_LOAD_EXTENSION.name, info == null ? null : info.getProperty(ENABLE_LOAD_EXTENSION.name));
		ele.description = "Turn extension loading on or off.";
		ele.choices = new String[]{"on", "off"};
		if (ele.value == null) ele.value = "off"; // default

		DriverPropertyInfo encoding = new DriverPropertyInfo(ENCODING.name, info == null ? null : info.getProperty(ENCODING.name));
		encoding.description = "Set the encoding.";
		encoding.choices = new String[]{"UTF-8", "UTF-16", "UTF-16le", "UTF-16be"};

		DriverPropertyInfo df = new DriverPropertyInfo(DateUtil.DATE_FORMAT, info == null ? null : info.getProperty(DateUtil.DATE_FORMAT));
		...
		DriverPropertyInfo tf = new DriverPropertyInfo(DateUtil.TIME_FORMAT, info == null ? null : info.getProperty(DateUtil.TIME_FORMAT));
		tf.description = "Specify the format used to persist time ('" + DateUtil.JULIANDAY + "', '" + DateUtil.UNIXEPOCH + "', 'HH:mm:ss.SSSXXX', '...').";
		DriverPropertyInfo tsf = new DriverPropertyInfo(DateUtil.TIMESTAMP_FORMAT, info == null ? null : info.getProperty(DateUtil.TIMESTAMP_FORMAT));
	...
	private static SQLWarning setup(org.sqlite.Conn conn, Properties info) throws ConnException {
		...
		String triggers = info.getProperty(ENABLE_TRIGGERS.name);
