...
class Stmt implements Statement {
	...
	int findCol(@NonNull String col) throws SQLException {
	...
	@Nullable
	private Integer findColIndexInCache(@NonNull String col) {
	...
	private void addColIndexInCache(@NonNull String col, int index, int columnCount) {
		if (null == colIndexByName) {
	...
	boolean step(boolean updateOnly) throws SQLException {
		int res = stmt.stepNoCheck(queryTimeout);
		boolean hasRow = res == SQLite.SQLITE_ROW;
		boolean hasRowOrColumn = hasRow || stmt.getColumnCount() != 0;
	...
	public int executeUpdate(@NonNull String sql, int autoGeneratedKeys) throws SQLException {
		Generated generated = Conn.checkAutoGeneratedKeys(autoGeneratedKeys);
		if (prepared) {
			throw new SQLException("method not supported by PreparedStatement");
		} else {
			_close();
	...
	@Override
	public int executeUpdate(@NonNull String sql, String @Nullable[] columnNames) throws SQLException {
	...
	@Override
	public boolean execute(@NonNull String sql, int @Nullable[] columnIndexes) throws SQLException {
		if (empty(columnIndexes)) {
			return execute(sql);
		}
	...
	@Override
	public boolean execute(@NonNull String sql, String @Nullable[] columnNames) throws SQLException {
		if (empty(columnNames)) {
			return execute(sql);
		}
		// https://sqlite.org/lang_returning.html
